...
--- Seal.plugins.filesearch.displayNoResultsTimeout
--- Variable
--- Timeout extension in cas no results were found after timeout. Defaults to 0.4s (400ms).
---
--- Notes:
---  * higher value might more results but will give a less snappy experience
obj.displayResultsExtendedTimeout = 0.8
...
obj.fileSearchOptions = {
    ...
    extendedSearchTimeout = obj.displayResultsExtendedTimeout,
...
local log = hs.logger.new('seal_filesearch', 'debug')
...
function SpotlightFileSearch:new(query, callback, options)
    ...
    self.extendedSearchTimeout = options.extendedSearchTimeout
...
function SpotlightFileSearch:runCallback(forbidExtension)
    ...
    if not forbidExtension and #self.searchResults == 0 and self.extendedSearchTimeout then
        log.d("No result after timeout, extending search timeout by " .. self.extendedSearchTimeout)
        if self.searchTimer ~= nil then
            self.searchTimer:stop()
        end
        local runCallback = hs.fnutils.partial(self.runCallback, self, true)
        self.searchTimer = hs.timer.doAfter(self.extendedSearchTimeout, runCallback)
...
function openInFinder(path)
    ...
    local _, _, _, _ = hs.osascript.applescript(string.format(applescript, path))
end

function obj.completionCallback(rowInfo)
    if rowInfo["type"] == "open" then
        local filePath = rowInfo["path"]
        if hs.eventtap.checkKeyboardModifiers()['alt'] then
            hs.task.new("/usr/bin/open", nil, {"--reveal", filePath}):start()
        elseif string.find(filePath, "%.applescript$") or string.find(filePath, "%.scpt$") then
            hs.task.new("/usr/bin/osascript", nil, {filePath}):start()
        else
            -- log.d("focused " .. hs.inspect.inspect(chooser._lastFocused))
            -- chooser._lastFocused[obj.seal.chooser] = nil
            hs.task.new("/usr/bin/open", nil, {filePath}):start()
        end
    end
end
